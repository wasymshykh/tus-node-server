...
class FileStore extends DataStore {
    constructor(options) {
        ...
        const config_file_id = `${pkg.name}-${pkg.version}`;
        this.configstorePath = options.configstorePath ? path.join(options.configstorePath, `${config_file_id}.json`) : false;
        this.extensions = ['creation', 'creation-with-upload', 'creation-defer-length', 'termination']
        if (this.configstorePath) {
            this.configstore = new Configstore(config_file_id, null, { configPath: this.configstorePath })
        } else {
            this.configstore = new Configstore(config_file_id);
        }
